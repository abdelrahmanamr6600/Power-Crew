...
class LoginRepository(private val firebaseAuth: FirebaseAuth, private val fireStore: FirebaseFirestore ,private val context: Context) {
    ...
    suspend fun login(email: String, password: String): Resource<User> {
        return try {
            ...
            val userQuery =
                fireStore.collection(FirestoreCollections.USER).document(result.user!!.uid).get()
                    .await()
            val user = userQuery.toObject(User::class.java)
            saveUserDataToDataStore(user!!)
            editLoginState(true)
            saveUserToken()
            changeUserState(user.userId)
    ...
    private  fun changeUserState(userId:String){
        FirestoreInstance.db
            .collection(FirestoreCollections.USER)
            .document(userId)
            .update(FirestoreFieldNames.STATE, true)
    }
